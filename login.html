...
<div class="login-container">
  <div class="login-header">
    ...
    <h1 class="login-title">MedWard Pro</h1>
    <p class="login-subtitle">Clinical Ward Management System</p>
  </div>
  ...
  <div class="login-card">
    ...
    <button class="btn btn-secondary" onclick="showRegister()" id="registerToggle">
      Create New Account
    </button>
    ...
    <form id="registerForm" onsubmit="handleRegister(event)" style="display:none">
      <div class="form-group">
        ...
        <input type="text" class="form-input" id="regUsername" placeholder="Choose a username" autocomplete="username" required minlength="3">
        <div class="error-text">Username must be at least 3 characters</div>
      </div>

      <div class="form-group">
        <label class="form-label">Create Password</label>
        <input type="password" class="form-input" id="regPassword" placeholder="Create a password" autocomplete="new-password" required minlength="4">
        <div class="error-text">Password must be at least 4 characters</div>
      </div>

      <div class="form-group">
        <label class="form-label">Confirm Password</label>
        <input type="password" class="form-input" id="regConfirm" placeholder="Confirm password" autocomplete="new-password" required>
...
document.getElementById('themeBtn').textContent = document.body.classList.contains('dark-theme') ? '☀️' : '🌙';
...
async function handleLogin(e) {
  ...
  try {
    ...
    if (result.success) {
      // Store credentials
      sessionStorage.setItem('MW_USER', username);
      sessionStorage.setItem('MW_PASS', password);
      
      if (remember) {
        localStorage.setItem('MW_USER', username);
        localStorage.setItem('MW_PASS', password);
      }
      ...
      showMessage(result.error || 'Invalid username or password');
    }
  } catch (error) {
...
async function handleRegister(e) {
  ...
  if (username.length < 3) {
    document.getElementById('regUsername').parentElement.classList.add('error');
    return;
  } else {
    document.getElementById('regUsername').parentElement.classList.remove('error');
  }
  
  if (password.length < 4) {
    document.getElementById('regPassword').parentElement.classList.add('error');
    return;
  } else {
    document.getElementById('regPassword').parentElement.classList.remove('error');
  }
  
  if (password !== confirm) {
    document.getElementById('regConfirm').parentElement.classList.add('error');
    return;
  } else {
    document.getElementById('regConfirm').parentElement.classList.remove('error');
  }
